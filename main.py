"""
Desktop Bluetooth Keyboard - Main Application
Cross-platform GUI application to use desktop as Bluetooth keyboard
"""
import asyncio
...
try:
    from bluetooth_keyboard import BluetoothKeyboardService, BluetoothDeviceInfo
    BLUETOOTH_AVAILABLE = True
except ImportError as e:
    BLUETOOTH_AVAILABLE = False
    BluetoothKeyboardService = None
...
class BluetoothKeyboardApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Desktop Bluetooth Keyboard")
        ...
        self.bluetooth_service = BluetoothKeyboardService()
    ...
    def show_installation_error(self):
        """Show error message about missing bleak library"""
        error_frame = ttk.Frame(self.root, padding="20")
        error_frame.pack(fill=tk.BOTH, expand=True)
        
        title = ttk.Label(error_frame, text="Bluetooth Library Not Installed", 
                         font=("Arial", 16, "bold"))
        title.pack(pady=10)
        
        error_text = f"""No Bluetooth library found. Please install one of the following:

Error: {IMPORT_ERROR}

Installation Options (try in order):

1. SIMPLEBLE (Recommended - easiest to install):
   pip install simpleble
   
2. BLEAK (Most common, but may fail on MSYS2 Python):
   pip install bleak
   Note: If using MSYS2 Python, use standard Windows Python instead
   
3. PYBLUEZ (Windows/Linux, classic Bluetooth):
   pip install pybluez

RECOMMENDED: Use Standard Windows Python
- Download from https://www.python.org/downloads/
- Check "Add Python to PATH" during installation
- Then try: pip install simpleble (easiest option)

The app will automatically use whichever library is available."""
